...
@mcp.tool()
async def get_inferred_links(analysis_id: str) -> dict:
    """
    Get the inferred conversational-link edge list: pairs of messages the
    backend judged likely to be part of the same conversation despite
    carrying no explicit reply signal (no Slack thread_ts, no WhatsApp
    reply), scored on semantic similarity, temporal proximity, speaker
    turn-taking, and shared keywords. Each edge also appears inline on
    query_chat results as `linked_context`; this tool returns the full edge
    ...
    async with httpx.AsyncClient() as client:
        try:
             response = await client.get(f"{API_BASE_URL}/graph/{analysis_id}/inferred-links")
        except Exception as e:
             return {"error": "Connection failed", "detail": str(e)}
