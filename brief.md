...
The core pipeline is complete and tested. Given a list of structured messages it correctly builds a directed graph, calculates influence metrics, detects communities, extracts topics locally, and produces a standalone interactive HTML visualisation with filtering controls.

What does not exist yet is everything around it — no file ingestion, no API, no MCP server, no web UI. Everything currently runs from hardcoded sample messages in the demo script.
